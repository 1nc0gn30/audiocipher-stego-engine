"""
Cross-platform compatibility utilities for audiocipher-stego-engine.
...
def get_default_storage_dir() -> Path:
    """Determine OS-specific default storage directory for audio cipher artifacts."""
    if IS_TERMUX:
        base = Path(os.environ.get("HOME", "/data/data/com.termux/files/home")) / ".audiocipher"
    elif IS_WINDOWS:
        base = Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData" / "Local")) / "AudioCipher"
    elif IS_MACOS:
        base = Path.home() / "Library" / "Application Support" / "AudioCipher"
    else:
        base = Path.home() / ".local" / "share" / "audiocipher_stego"
